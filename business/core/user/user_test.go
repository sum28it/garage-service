...
import (
	...
	"github.com/sum28it/garage-service/business/data/dbtest"
	"github.com/sum28it/garage-service/foundation/docker"
)
...
func Test_User(t *testing.T) {
	...
	t.Log("Given the need to work with User records.")
	{
		...
		t.Logf("\tTest %d:\tWhen handling a single User.", testID)
		{
			...
			email, err := mail.ParseAddress("test@example.com")
			if err != nil {
			...
			email, err = mail.ParseAddress("test@sheep.com")
			if err != nil {
				t.Fatalf("\t%s\tTest %d:\tShould be able to parse email: %s.", dbtest.Failed, testID, err)
			}
			t.Logf("\t%s\tTest %d:\tShould be able to parse email.", dbtest.Success, testID)
			...
			upd := user.UpdateUser{
				Name:     dbtest.StringPointer("Test Sheep"),
				Email:    email,
